...
if __name__ == "__main__": 
    ...
    # Map the ram and storage categories to integers
    df['ram'] = df['ram'].str.replace('inbuilt', '')
    df['ram_int'] = df['ram'].astype('category').cat.codes

    # Scatter plot of the data points
    fig, ax = plt.subplots(figsize=(8, 6))
    plt.scatter(x=df['ram'], y=df['Price_Euro'], s=2)
    ...
    df['centroid'], df['error'], centroids =  kmeans(df[['ram_int','Price_Euro']], k)
    c = df['centroid']
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    plt.scatter(x=df['ram_int'], y=df['Price_Euro'],  marker = 'o', 
